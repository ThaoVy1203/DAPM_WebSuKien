...
CREATE TABLE SuKien (
    ...
    gioHuyTruocBatDauPhut INTEGER NOT NULL DEFAULT 120,
    yeuCauKhaoSatCheckout BIT NOT NULL DEFAULT 1,
...
-- 16. BẢNG DangKyDanhGia (BẢNG MỚI ĐƯỢC THÊM VÀO ĐỂ ĐỒNG BỘ)
IF OBJECT_ID('DangKyDanhGia', 'U') IS NOT NULL
    DROP TABLE DangKyDanhGia;
GO

CREATE TABLE DangKyDanhGia (
    idDanhGia INTEGER NOT NULL IDENTITY(1,1),
    idDangKy INTEGER NOT NULL,
    ...
    CONSTRAINT FK_DKDG_DangKy FOREIGN KEY (idDangKy) REFERENCES DangKySuKien(idDangKy) ON DELETE CASCADE,
    CONSTRAINT UQ_DKDG_DangKy UNIQUE (idDangKy)
);
GO
...
(
    ...
    1, 'ND002', 300, '../images/event1.png', N'Đã duyệt', 'ND005'
),
(
...
UNION ALL SELECT 'DangKyDanhGia', COUNT(*) FROM DangKyDanhGia;
